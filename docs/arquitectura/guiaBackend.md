...
- **Arquitectura MVC:** Se respeta la separación de responsabilidades entre Modelos, Vistas (JSON responses) y Controladores.
- **Servicios:** La lógica de negocio compleja se delega a la capa de servicios (`services/`) para mantener los controladores ligeros.
- **Seguridad:** Uso de middlewares para validación de tokens (JWT) y roles de usuario.
