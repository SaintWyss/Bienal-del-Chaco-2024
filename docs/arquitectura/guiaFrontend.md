...
- Contiene módulos específicos relacionados con funcionalidades clave del proyecto, organizados por características (Feature-based Folder Structure).
- Cada feature (ej. `auth`, `admin`, `home`) puede contener su propia estructura interna:
  - **`components/`**: Componentes de UI específicos de la feature.
  - **`pages/`**: Componentes de página que actúan como vistas principales.
  - **`hooks/`**: Hooks personalizados específicos de la feature.
  - **`services/`**: (Opcional) Servicios específicos si no son compartidos.
- **Nota:** Los componentes principales incluyen encabezados CRC (Class-Responsibility-Collaborator) para facilitar la comprensión de su propósito.
